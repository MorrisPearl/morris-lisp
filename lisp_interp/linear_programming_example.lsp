...
; concentration, average duration, and credit risk. The file names each
; pool's variable (gnma_30, fnma_30, ...) and uses the names in the
; formulas, so no rows of zeros are needed; and it says "maximize", so the
...
; The names from the file, in the order of the objective's coefficients
; and of the solution. Here the objective's coefficients are the yields.
(define pool-names (cdr (assoc "variables" problem)))
(define pool-yields (cdr (assoc "objective" problem)))
...
; The problem says "maximize", so this is the most income there can be.
...
Every part is kept as it is, except the rhs."
  (define rhs (cdr (assoc "rhs" problem)))
  (map (lambda (part)
         (if (equal? (car part) "rhs")
